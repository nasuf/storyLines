...
import com.story.repository.MessageRepository;
...
import com.story.repository.TagRepository;
...
public class StoryController {
	...
	public @ResponseBody ResponseEntity<Map<String, Object>> addPhase(@RequestBody Phase phaseObject,
...
//			this.messageService.notify(Constant.MESSAGE_TYPE_MULTIPLE, null, null, null, null, null, null);
			...
			if (!updatedParentPhase.getAuthorOpenid().equals(openid)) {
				...
				if (updatedParentPhase.getAuthorOpenid() != openid); // don't send to myself if the parent phase author is myself
	 ...
	 */
	/*
	 * @RequestMapping(value = "/phase/{phaseLevel}", method =
	 * RequestMethod.GET) public ResponseEntity<Map<String, Object>>
	 * findPhases(@PathVariable("phaseLevel") Integer phaseLevel,
	 * 
	 * @RequestParam("storyId") String storyId) { if
	 * (StringUtils.isEmpty(phaseLevel)) { return new ResponseEntity<Map<String,
	 * Object>>( new HttpResult(Constant.RESULT_STATUS_FAILURE,
	 * "Phase level can't be null").build(), HttpStatus.BAD_REQUEST); } if
	 * (StringUtils.isEmpty(storyId)) { return new ResponseEntity<Map<String,
	 * Object>>( new HttpResult(Constant.RESULT_STATUS_FAILURE,
	 * "Story id can't be null").build(), HttpStatus.BAD_REQUEST); } List<Phase>
	 * foundPhases = this.phaseRepository.findByStoryIdAndLevel(storyId,
	 * phaseLevel); Map<String, Object> data = new HashMap<String, Object>();
	 * data.put(Constant.RESULT_DATA, foundPhases); return new
	 * ResponseEntity<Map<String, Object>>(new
	 * HttpResult(Constant.RESULT_STATUS_SUCCESS, "Found " + foundPhases.size()
	 * + " phases for story id: [" + storyId + "] and phase level: [" +
	 * phaseLevel + "]", data).build(), HttpStatus.OK); }
	 */
